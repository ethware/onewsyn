...
import settings as stts #根据st_template.py设计及settings

#复制文件后，直接使用os.utime仍不能正确同步大部分文件时间戳，samba模式下，文件内容同步完成后，统一修改时间戳的方法
#2021-05-24

#支持排除功能，跳过对所选路径的备份与同步； [考虑是否需要在对比时统一转换为大写或者小写,暂时不转换]
