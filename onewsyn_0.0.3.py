...
def onewsyn(path_src, path_dst='HELLO_BACKUP', samba=''):
    ...
    print('Add %d files, Updated %d files, and Removed %d entries.' %(static[0], static[1], static[2]))
...
def forward_update(path_src, path_dst):
    ...
    for item_src in dirs_src:
        ...
        elif not os.path.isdir(os.path.join(path_src, item_src)):
            
            update_file(os.path.join(path_src, item_src), os.path.join(path_dst, item_src))

        elif os.path.isdir(os.path.join(path_src, item_src)) and (not os.path.islink(item_src)):
            forward_update(os.path.join(path_src, item_src), os.path.join(path_dst, item_src))

        elif os.path.islink(item_src):
            add_orgy(os.path.join(path_src, item_src), os.path.join(path_dst, item_src))
...
def backward_clean(path_src, path_dst):
    ...
    for item_dst in dirs_dst:
        ...
        elif os.path.isdir(os.path.join(path_dst, item_dst)) and (not os.path.islink(os.path.join(path_dst, item_dst))):
            backward_clean(os.path.join(path_src, item_dst), os.path.join(path_dst, item_dst))

        elif os.path.islink(os.path.join(path_dst, item_dst)):
            pass
            #remove_orgy(os.path.join(path_dst, item_dst))
